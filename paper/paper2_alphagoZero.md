...
- 首卷积模块，提取输入特征
  ...
  $`19 \times 19 \times 17 \rightarrow \mathrm{Conv}_{3 \times 3}(256)`$
...
- **损失函数**：
  ...
  $`\begin{aligned} L(\theta) &= (z-v)^2 - \pi^\top \log p + c\lVert\theta\rVert^2, \\ L_v &= (z-v)^2, \\ L_p &= -\pi^\top\log p, \\ L_{\mathrm{reg}} &= c\lVert\theta\rVert^2 \end{aligned}`$
